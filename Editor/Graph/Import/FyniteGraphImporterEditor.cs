...
namespace Fynite.GraphEditor
{
    ...
    internal sealed class FyniteGraphInspectorSession
    {
        ...
        internal void Apply()
        {
            ...
            // Graph Toolkit 0.8 only marks its file dirty for public structural operations; changing a
            // serialized field owned by the derived Graph is intentionally outside its change model.
            // Touch that public model without leaving an element behind so SaveGraph serializes the
            // graph-wide metadata too. No internal API or reflection is involved.
            var changeMarker = new FyniteRootStateNode();
            Graph.AddNode(changeMarker);
            Graph.RemoveNode(changeMarker);
